<?php
/**
     * @class magiccontentWidget
     * @author Eundong (dh1024@gmail.com)
     * @brief Content 위젯의 magiccontent 지원 버전
...
class magiccontentWidget extends WidgetHandler
{
	...
	function proc($args)
	{
		$args->widget = 'magiccontentWidget';
		$args->widgetContentItem = 'magiccontentItem';

		Context::set('widget_sequence', $args->widget_sequence);
		...
		Context::set('magiccontent_content_items', $content_items);
		$output = $this->_compile($args,$content_items);
		$oTemplate = &TemplateHandler::getInstance();
		$tpl_path = sprintf('%stpl', $this->widget_path);
		$header = $oTemplate->compile($tpl_path, "_header");
		$footer = $oTemplate->compile($tpl_path, "_footer");
	...
	function _getDocumentItems($args)
	{
		...
		if(count($output->data))
		{
			...
			for($i=0,$c=count($document_srls);$i<$c;$i++)
			{
				...
				$content_item = new magiccontentItem( $args->module_srls_info[$module_srl]->browser_title );
	...
	function _getImageItems($args)
	{
		...
		foreach($tmp_document_list as $oDocument)
		{
			...
			$content_item = new magiccontentItem($browser_title);
	...
	function _getRssItems($args)
	{
		...
		if($xml_doc->rss)
		{
			...
			foreach ($items as $key => $value)
			{
				...
				$content_item = new magiccontentItem($rss->title);
...
class magiccontentItem extends Object
{
	...
	function magiccontentItem($browser_title='')
	{
